...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    if std::env::var("TEST_LOG").is_ok() {
        let subscriber = get_subscriber(
            subscriber_name, default_filter_level,
             std::io::stdout
        );
...
impl TestApp {
    ...
    pub fn get_confirmation_link(&self, email_request: &wiremock::Request) -> ConfirmationLinks {
        ...
        let get_link = |s: &str| {
            ...
            let _ = confirmation_link.set_port(Some(self.port));
            confirmation_link
        };
    ...
    pub async fn post_newsletters<Body>(&self, body: &Body, is_authenticated: bool) -> reqwest::Response 
    where
