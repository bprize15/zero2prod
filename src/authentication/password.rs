...
pub async fn validate_credentials(credentials: Credentials, pool: &PgPool) -> Result<uuid::Uuid, AuthError> {
    ...
    if let Some((stored_user_id, stored_password_hash)) = 
        get_stored_credentials(&credentials.username, pool)
            .await
            .map_err(AuthError::UnexepectedError)?
    {
...
pub async fn change_password(user_id: uuid::Uuid, password: Secret<String>, pool: &PgPool) -> Result<(), anyhow::Error> {
...
fn verify_password_hash(expected_password_hash: Secret<String>, password_candidate: Secret<String>) -> Result<(), AuthError> {
